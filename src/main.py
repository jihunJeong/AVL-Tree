...
class AVL_Tree:
    ...
    def setHeight(self, node, cnode):
        # node is parent node, cnode is child node
        node.height = max(self.getHeight(node.left), self.getHeight(node.right)) + 1
        cnode.height = max(self.getHeight(cnode.left), self.getHeight(cnode.right)) + 1

    def difHeight(self, node):
        # Different height between left and right child for rotate information
        return self.getHeight(node.left) - self.getHeight(node.right)

    def balance(self, node):
        # Keep AVL Tree structure function
        dif = self.difHeight(node)

        if dif > 1:
            if self.difHeight(node.left) < 0:
                node = self.rotateLR(node)
            else :
                node = self.rotateLL(node)
        elif dif < -1:
            if self.difHeight(node.right) > 0 :
